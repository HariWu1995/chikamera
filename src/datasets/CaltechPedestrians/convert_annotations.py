...
for dname in sorted(glob('original/annotations/set*')):

    set_name = os.path.basename(dname)
    data[set_name] = defaultdict(dict)

    for anno_fn in sorted(glob('{}/*.vbb'.format(dname))):
        ...
        for frame_id, obj in enumerate(objLists):
            if len(obj) <= 0:
                continue
            for id, pos, occl, lock, posv in zip(obj['id'][0], obj['pos'][0], obj['occl'][0],
...
json.dump(data, open('preprocessed/annotations.json', 'w'))#, indent=4)
