...
class _BIPSolver:
    ...
    def _create_bip(affinity_matrix, min_affinity, max_affinity):
        ...
        if n_nodes >= 3:
            ...
            constraints_edges_idx[:, 0] = nodes_to_edge_matrix[(triplets[:, 0], triplets[:, 1])]
            constraints_edges_idx[:, 1] = nodes_to_edge_matrix[(triplets[:, 0], triplets[:, 2])]
            constraints_edges_idx[:, 2] = nodes_to_edge_matrix[(triplets[:, 1], triplets[:, 2])]
            constraints_edges_idx = constraints_edges_idx[np.any(constraints_edges_idx >= 0, axis=1)]
    ...
    def solve(self, affinity_matrix, rtn_matrix=False):
        ...
        if n_nodes <= 1:
            ...
            # solve
            solution_x = self._solve_bip(objective_coefficients, sparse_constraints, upper_bounds)
            ...
            sol_matrix[np.triu(np.ones([n_nodes, n_nodes], dtype=int), 1) > 0] = all_sols
