...
def _iou(a_tracks, b_tracks):
    ious = np.zeros((len(a_tracks), len(b_tracks)), dtype=np.float64)
    if ious.size == 0:
        return ious
    ...
    or (len(b_tracks) > 0 and isinstance(b_tracks[0], np.ndarray)):
        ...
        altrbs = [track.ltrb for track in a_tracks]
        bltrbs = [track.ltrb for track in b_tracks]
    ...
    outl = np.minimum(pl, gl)
    outt = np.minimum(pt, gt)
    outr = np.minimum(pr, gr)
    outb = np.maximum(pb, gb)
...
def GIoU_np(bbox_p, bbox_g):
    ...
    I = np.maximum((y2I - y1I), axis=0) * np.maximum((x2I - x1I), axis=0)
...
def bboxes_ciou(tracks1, tracks2):
    ...
    # calulate the box's area of boxes1 and boxess
...
def v_iou_distance(a_tracks, b_tracks):
    ...
    or (len(b_tracks) > 0 and isinstance(b_tracks[0], np.ndarray)):
        ...
        altrbs = [track.ltwh_to_ltrb(track.pred_bbox) for track in a_tracks]
        bltrbs = [track.ltwh_to_ltrb(track.pred_bbox) for track in b_tracks]
    _ious = ious(altrbs, bltrbs)
    cost_matrix = 1 - _ious
    return cost_matrix
...
def embedding_distance(tracks, detections, metric='cosine'):
    ...
    if cost_matrix.size == 0:
        return cost_matrix
    detect_features = np.asarray([track.curr_feat for track in detections], dtype=np.float64)
    track_features = np.asarray([track.smooth_feat for track in tracks], dtype=np.float64)
    cost_matrix = np.maximum(0.0, cdist(track_features, detect_features, metric))  # Nomalized features
    return cost_matrix
