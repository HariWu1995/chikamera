...
from ..utils.misc import pack_seqdim, unpack_seqdim, matmul2, reduce_masked_mean
from ..utils.intrinsics import scale_intrinsics
...
class Liftnet(nn.Module):
    ...
    def __init__(self, Y, Z, X,
        ...
        self.rgb_compress = rgb_compress
        ...
        if encoder_type == 'res101':
            ...
            self.encoder = Encoder_res34(feat2d_dim + self.D)
    ...
    def create_frustum(self):
        ...
        fH = ogfH // self.downsample
        fW = ogfW // self.downsample
    ...
    def get_geometry(self, rots, trans, intrins, post_rots, post_trans):
        ...
        Returns 
            (B, N x D x H/downsample x W/downsample x 3)
        """
        B, N, _ = trans.shape
    ...
    def forward(self, rgb_cams, pix_T_cams, cams_T_global, vox_util, ref_T_global, prev_bev=None):
        ...
        if self.rand_flip:
            ...
            rgb_cams_[rgb_flip_index] = torch.flip(rgb_cams_[rgb_flip_index], [-1])
        
        feat_cams_ = self.encoder(rgb_cams_)  # S, (D+C), H/8, W/8

        if self.rand_flip:
            feat_cams_[rgb_flip_index] = torch.flip(feat_cams_[rgb_flip_index], [-1])
        ...
        depth_cams_out = feat_cams_[:, :self.D].unsqueeze(1)    # B, 1, D, Hf, Wf
        feat_cams_     = feat_cams_[:, self.D:].unsqueeze(2)    # B, C, 1, Hf, Wf

        # depth_camXs_ = (depth_camXs_out / 0.07)
        depth_cams_ = depth_cams_out.softmax(dim=2)     # B, 1, D, Hf, Wf
        feat_tileXs_ = feat_cams_ * depth_cams_         # B, C, D, Hf, Wf

        # unproject image feature to 3d grid
        feat_mems_ = vox_util.warp_tiled_to_mem(
                    feat_tileXs_,
            matmul2(featpix_T_cams_, cams_T_ref_),  # featpix_T_ref
                    cams_T_ref_, 
                    Y, Z, X, 
                    self.DMIN, 
                    self.DMAX+self.DMIN, 
        ...
        if self.num_cameras is None:
            one_mems_ = vox_util.warp_tiled_to_mem(
                    torch.ones_like(feat_tileXs_),
                            matmul2(featpix_T_cams_, cams_T_ref_),
                            cams_T_ref_, 
                            Y, Z, X, 
                            self.DMIN, 
                            self.DMAX+self.DMIN, 
            ...
            feat_mem = reduce_masked_mean(feat_mems, one_mems, dim=1)  # B, C, Y, Z, X
        else:
        ...
        out_dict = self.decoder(bev_features, feat_cams_.squeeze(2),
                                (bev_flip1_index, bev_flip2_index) if self.rand_flip else None)
