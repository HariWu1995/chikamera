...
from ..utils.misc import pack_seqdim, unpack_seqdim, reduce_masked_mean, DEVICE
...
class MVDet(nn.Module):
    ...
    def forward(self, rgb_cams, pix_T_cams, cams_T_global, vox_util, ref_T_global, prev_bev=None):
        ...
        featpix_T_cams_ = scale_intrinsics(pix_T_cams_, sx, sy)  # B*S,4,4
        featpix_T_ref_ = torch.matmul(featpix_T_cams_[:, :3, :3], 
                                          cams_T_ref_[:, :3, [0, 1, 3]])  # B*S,3,3
