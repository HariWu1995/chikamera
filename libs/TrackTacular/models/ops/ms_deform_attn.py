...
from mmcv.ops.multi_scale_deform_attn import MultiScaleDeformableAttnFunction as MSDeformAttnF
...
class MSDeformAttn(nn.Module):
    ...
    def __init__(self, d_model: int = 256, n_levels: int = 4, 
        ...
        if not _is_power_of_2(_d_per_head):
            warnings.warn(
                "You'd better set d_model in MSDeformAttn to "
    ...
    def forward(self, query, reference_points, 
        ...
        output = MSDeformAttnF.apply(value, input_spatial_shapes, input_level_start_index, 
...
class MSDeformAttn3D(nn.Module):
    ...
    Args:
        embed_dims (int = 256): The embedding dimension of Attention.
        num_heads (int = 64): Parallel attention heads.
        num_levels (int = 4): The number of feature map used in Attention.
        num_points (int = 4): The number of sampling points for each query in each head.
        im2col_step (int = 64): The step used in image_to_column.
        dropout (float = 0.1): A Dropout layer on `inp_identity`.
        batch_first (bool = False): K, Q and V are shape of (batch, n, embed_dim)
                                                         or (n, batch, embed_dim).
        norm_cfg (dict = None                  ): Config dict for normalization layer.
        init_cfg (obj: `mmcv.ConfigDict` = None): Config dict for initialization.
    """
    ...
    def __init__(self, embed_dims: int = 256, num_heads: int = 8, 
                        ...
                        batch_first: bool = True, 
                            norm_cfg: dict = None, init_cfg = None):
        super().__init__()
    ...
    def forward(self,
        ...
        Args:
            query (Tensor): Query of Transformer with shape `(bs, num_query, embed_dims)`.
              key (Tensor):   Key of Transformer with shape `(bs, num_key, embed_dims)`.
            value (Tensor): Value of Transformer with shape `(bs, num_key, embed_dims)`.
            ...
            reference_points (Tensor): 
                The normalized reference points 
                with shape (bs, num_query, num_levels, 2),
                    all elements is in range [0, 1], top-left (0,0), bottom-right (1,1), 
                    including padding area.
                Or shape (N, Length_{query}, num_levels, 4), 
                    add additional 2 dimensions is (w, h) to form reference boxes.
            ...
            level_start_index (Tensor): The start index of each level.
                A tensor has shape ``(num_levels, )`` and 
        ...
        output = MSDeformAttnF.apply(value, spatial_shapes, level_start_index, 
                                    sampling_locations, attention_weights, self.im2col_step)
