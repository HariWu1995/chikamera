...
from ..utils.misc import img_transform
...
class SynthehicleDataset(VisionDataset):
    ...
    def setup(self):
        if self.split != 'test':
            town_paths = glob(osp.join(self.root, 'train', 'Town*-O-*'))
        else:
            town_paths = glob(osp.join(self.root, self.split, 'Town*-O-*'))
        ...
        for town_num, town_path in enumerate(sorted(town_paths)):
            ...
            # Add samples: 70% train - 30% eval
            frames = glob(osp.join(town_path, camera_names[0], 'out_rgb', '*.jpg'))
            samples = [
                (town_num, town_name, town_path, osp.basename(f)[:-4]) 
            ...
            cams_T_global = merge_rt(rots, trans.squeeze(-1))   # S, 4, 4
            global_T_cams = torch.inverse(cams_T_global)        # S, 4, 4
            cx, cy = global_T_cams[:, :2, 3].mean(dim=0)
            self.scene[town_name] = {
                'scene_center': torch.tensor([cx, cy, 0.0]),
            }
    ...
    def get_image_data(self, index, cameras, bbox_cam):
        ...
        for camera in cameras.values():
            ...
            center_img, offset_img, \
                size_img, pid_img, valid_img = self.get_img_gt(bbox, sx, sy, crop)
        ...
        return (torch.stack(imgs), torch.stack(rots), torch.stack(trans), 
                torch.stack(intrins), torch.stack(centers),
    ...
    def sample_augmentation(self):
        ...
        if self.is_train:
            ...
            crop_offset = 0.5 * int(self.data_aug_conf['resize_lim'][0] * \
    ...
    def get_bbox_global(self, index):
        ...
        for _, cam in cameras.items():
            ...
            vehicle_data = zip(raw_data['vehicle_id'], 
                               raw_data['vehicle_class'], 
                               raw_data['world_coords'])
            for v_id, v_class, v_bbox in vehicle_data:
                # only passenger_cars, trucks, and motorbikes
                ...
                bbox_ids.add(v_id)
                bbox_global.append(torch.tensor(v_bbox))

        if len(bbox_global) > 0:
    ...
    def get_bev_gt(self, corners_ref, labels, corners_prev, labels_prev):
        """
        corners_ref: 1,N,8,3
            seg_bev: 1,Y,X
        """
        ...
        center = torch.zeros((C, self.Y, self.X), dtype=torch.float32)
        ...
        mask   = torch.zeros((1, self.Y, self.X), dtype=torch.bool)
        ...
        for center_pt, lwh, yaw, label in zip(mem_pts, lwhs[0], yaws[0], labels):
            ...
            draw_umich_gaussian(center[label['class']], ct_int, self.kernel_size)
        ...
        return center, mask, ids, offset, size, rotbin, rotres
    ...
    def get_img_gt(self, bbox, sx, sy, crop):
        ...
        for (pts, pid, idx) in zip(img_pts, img_pids, classes):
            ...
            draw_umich_gaussian(center[idx], ct_int, self.kernel_size)
    ...
    def __getitem__(self, index) -> Any:
        town_num, town_name, town_path, \
            frame = self.samples[index]
        cameras = self.cameras[town_name]
        scene_center = self.scene[town_name]['scene_center']

        ref_T_global = merge_rt(torch.eye(3).unsqueeze(0), 
                                -scene_center.unsqueeze(0)).squeeze()
        if self.is_train:
        ...
        imgs, rots, trans, intrins, \
            centers_img, offsets_img, \
                sizes_img, tids_img, valids_img = self.get_image_data(index, cameras, bbox_cam)
        
        extrins = merge_rt(rots, trans.squeeze(-1))  # S 4 4
        ...
        center_bev, valid_bev, ids_bev, \
        offset_bev, size_bev, \
        rotbin_bev, rotres_bev = self.get_bev_gt(corners_ref, labels, corners_prev, labels_prev)
...
if __name__ == '__main__':
    ...
    datapath = '/usr/home/tee/Developer/datasets/synthehicle'
    data = SynthehicleDataset(datapath)
    print(len(data))
