...
from .utils.misc import reduce_masked_mean
...
class SimpleLoss(torch.nn.Module):
    ...
    def __init__(self, pos_weight: float):
        ...
        self.loss_fn = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([pos_weight]), reduction='none')
...
class FocalLoss(torch.nn.Module):
    ...
    def __init__(self, use_distance_weight: bool = False):
    ...
    def forward(self, pred, gt):
        ...
        if self.use_distance_weight:
            w, h = gt.shape[-2:]
            xs = torch.linspace(-1, 1, steps=h, device=device)
            ys = torch.linspace(-1, 1, steps=w, device=device)
