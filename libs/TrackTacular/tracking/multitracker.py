...
class STrack(BaseTrack):
    ...
    def multi_predict(stracks):
        if len(stracks) > 0:
            ...
            multi_mean, \
            multi_covariance = STrack.shared_kalman.multi_predict(multi_mean, multi_covariance)
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        self.mean, \
        self.covariance = self.kalman_filter.update(self.mean, self.covariance, new_track.xy)
    ...
    def update(self, new_track, frame_id, update_feature: bool = False):
        """
...
class JDETracker:
    """
    Joint Detection and Embedding (JDE) Tracking
    """
    ...
    def update(self, dets, dets_prev, score):
        ...
        if len(dets) > 0:
            detections = [STrack(xy, xy_prev, s, self.max_time_lost) 
        ...
        self.lost_stracks = dedup_stracks(self.tracked_stracks, self.lost_stracks)
...
def dedup_stracks(stracksa, stracksb):
