...
def mot_metrics(tSource, gtSource, scale: float = 0.025):
    ...
    for seq in np.unique(gt[:, 0]).astype(int):
        ...
        for frame in np.unique(gt[:, 1]).astype(int):
            ...
            # format: gt, t
            C = mm.distances.norm2squared_matrix(gt_dets[:, 1:3] * scale, 
                                                 dt_dets[:, 1:3] * scale, max_d2=1)
            C = np.sqrt(C)

            acc.update(gt_dets[:, 0].astype('int').tolist(),
                       dt_dets[:, 0].astype('int').tolist(), C, frameid=frame)
    ...
    summary_str = mm.io.render_summary(summary, formatters=mh.formatters,
