...
class PerspectiveTrainer(BaseTrainer):
    ...
    def __init__(self, model, logdir, cls_thresh=0.4, alpha=1.0, use_mse=False, id_ratio=0):
        super(BaseTrainer, self).__init__()
        self.model = model
    ...
    def train(self, epoch, dataloader, optimizer, scaler, scheduler=None, log_interval=100):
        ...
        for batch_idx, (data, world_gt, imgs_gt, affine_mats, frame) in pbar:
    ...
    def test(self, epoch, dataloader, res_fpath=None, visualize=False):
        ...
        for batch_idx, (data, world_gt, imgs_gt, affine_mats, frame) in pbar:
            ...
            if res_fpath is not None:
                ...
                for b in range(B):
                    ids = scores[b].squeeze() > self.cls_thresh
        ...
        if visualize:
            self.visualize(data, imgs_heatmap, world_heatmap, world_gt, epoch)
        ...
        if res_fpath is not None:
            ...
            recall, precision, \
                moda, modp = evaluate(os.path.abspath(res_fpath),
    ...
    def visualize(self, data, imgs_heatmap, world_heatmap, world_gt, epoch=None):
