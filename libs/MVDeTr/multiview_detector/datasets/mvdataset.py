...
class MVDataset(VisionDataset):
    ...
    def __init__(self, base, train=True, reID=False, 
                      world_reduce=4, world_kernel_size=10, 
                        img_reduce=12, img_kernel_size=10,
                       train_ratio=0.9, top_k=100, force_prepare=True,
        ...
        self.imgs_region = warp_perspective(world_masks, self.img_from_world, self.img_shape, 'nearest', align_corners=False)
        ...
        print(f'all: pid: {len(self.pid_dict)}, frame: {num_frame}, '
              f'keep ratio: {num_keep / num_all:.3f}\n'
    ...
    get_worldcoord_from_imgcoord_mat(self.base.intrinsic_matrices[cam],
                                     self.base.extrinsic_matrices[cam],
                                 z / self.base.worldcoord_unit) for cam in range(self.num_cam)
        ]

        # worldgrid(xy)_from_img(xy)
        proj_mats = [
            Rworldgrid_from_worldcoord_mat @ worldcoord_from_imgcoord_mats[cam] @ self.base.img_xy_from_xy_mat
            for cam in range(self.num_cam)
        ]
    ...
    def __getitem__(self, index, visualize=False):
        ...
        # imgs_gt['heatmap_mask'] = warp_perspective(imgs_gt['heatmap_mask'], affine_mats, self.img_shape, align_corners=False)
        # imgs_gt['heatmap_mask'] = F.interpolate(imgs_gt['heatmap_mask'], self.Rimg_shape, mode='bilinear', align_corners=False).bool().float()

        keep_cams = torch.ones(self.num_cam, dtype=torch.bool)
        drop = np.random.rand() < self.dropout
        if drop:
...
def test(test_projection=False):
    ...
    dataset = MVDataset(Wildtrack(os.path.expanduser('~/Data/Wildtrack')), train=True, augmentation=False)
    # dataset = MVDataset(MultiviewX(os.path.expanduser('~/Data/MultiviewX')), train=True)
    # dataset = MVDataset(Wildtrack(os.path.expanduser('~/Data/Wildtrack')), train=True, semi_supervised=.1)
    # dataset = MVDataset(MultiviewX(os.path.expanduser('~/Data/MultiviewX')), train=True, semi_supervised=.1)
    # dataset = MVDataset(Wildtrack(os.path.expanduser('~/Data/Wildtrack')), train=True, semi_supervised=0.5)
    # dataset = MVDataset(MultiviewX(os.path.expanduser('~/Data/MultiviewX')), train=True, semi_supervised=0.5)
    min_dist = np.inf
    ...
    xx, yy = np.meshgrid(np.arange(0, 1920, 20), 
                         np.arange(0, 1080, 20))
    H, W = xx.shape
    ...
    for cam in range(dataset.num_cam):
        ...
        for i, j in list(itertools.product(list(range(H)), list(range(W)))):
            x, y = world_grids[i, j]
