...
def create_reference_map(dataset, n_points=4, downsample=2, visualize=False):
    ...
    world_zoom_mat = np.diag([dataset.world_reduce * downsample, 
                              dataset.world_reduce * downsample, 1])
...
class MVDeTr(nn.Module):
    ...
    get_worldcoord_from_imgcoord_mat(dataset.base.intrinsic_matrices[cam],
                                     dataset.base.extrinsic_matrices[cam],
                                 z / dataset.base.worldcoord_unit)
            for cam in range(dataset.num_cam)
        ]
        ...
        self.proj_mats = torch.stack([torch.from_numpy(
    ...
    def forward(self, imgs, M, visualize=False):
        ...
        world_feat = warp_perspective(imgs_feat, proj_mats.to(imgs.device), self.Rworld_shape, align_corners=False).view(B, N, C, H, W)
        if visualize:
            for cam in range(N):
...
if __name__ == '__main__':
    ...
    dataset = MVDataset(
            Wildtrack(os.path.expanduser('~/Data/Wildtrack')), train=False, augmentation=False)
    ...
    (imgs_heatmap, imgs_offset, imgs_wh) = model(imgs, affine_mats)
