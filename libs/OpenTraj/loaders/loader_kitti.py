...
# loader for KITTI, 
# in global frame--origin is defined as the imu position at the first frame in that scene
...
def load_kitti(path, **kwargs):
    ...
    track_files_list = sorted(glob(path+"/label/*.txt"))
    calib_files_list = sorted(glob(path+"/calib/*.txt"))
    imu_files_list =  sorted(glob(path+"/oxts/*.txt"))
    
    # load track data, calibration data, IMU data from all scenes 
    track_rawData = load_track(track_files_list) # (left camera coordinate)
    calib_rawData = load_calib(calib_files_list)
    imu_rawData = load_IMU(imu_files_list)
    
    # convert track data to world coordinate (imu coordinate in the first frame of that scene)
    track_world_pos = track_cam_to_world(track_rawData, calib_rawData, imu_rawData) 
    ...
    # post-process. 
    # For KITTI, raw data do not include velocity
...
def load_track(files):
    csv_columns = ['frame', 'agent_id', 'type', 'truncated', 'occulded', 'alpha', 
                   'bbox_l', 'bbox_t', 'bbox_r', 'bbox_b', 'dimensions_h','dimensions_w','dimensions_l',
                   'location_x','location_y','location_z', 'rotation_y', 'score','scene']
    ...
    for file in files:
        ...
        # re-id
        copy_id = deepcopy(data["agent_id"])
...
def load_calib(files):
    calib_raw_dataset = []
    for file in files:
        calib = pd.read_csv(file, sep=" ", header=None) 
        calib_raw_dataset.append(calib.values[-2:, 1:-2]) # only keep 2 transformation rows
...
def convert_oxt_to_pose(raw_IMU,frame,Tr_0_inv=[]):
    '''
    converts a list of oxts measurements into metric poses, starting at (0,0,0) meters, 
    OXTS coordinates are defined as (see OXTS RT3000 user manual):
        x = forward, 
        y = right, 
        z = down

    afterwards, pose{i} contains the transformation which takes a 3D point in the i'th frame 
    and projects it into the oxts coordinates of the 1st frame.
    '''
    # compute scale from 1st lat value
    scale = math.cos(raw_IMU[0,0] * pi / 180.0) # 1st element in this row [lat]
...
def track_cam_to_world(track_rawData, calib_rawData, imu_rawData):
    ...
    for i in range(len(track_rawData)):
        ...
        # 1stly, convert it from cam coordinate to current IMU coordinate 
        ...
        _, Tr_0_inv = convert_oxt_to_pose(raw_IMU, 0)
        
        for j in track_pos_cam.values: # row
            track_pose_imu = np.matmul(Tr_velToimu, np.matmul(Tr_camTovel, np.r_[j[1:], [1]]))
            
            # Transform from current IMU coordinate to the IMU coordinate 
            #    in the 1st frame (world coordinate for this scene)
            Tr_imuToworld, _ = convert_oxt_to_pose(raw_IMU, int(j[0]), Tr_0_inv)
            track_pose_world_data.append(np.matmul(Tr_imuToworld, track_pose_imu).tolist()[0][:3])

    track_pos_world = np.array(track_pose_world_data)
    track_pos_world = pd.DataFrame(data=track_pos_world, columns=['pos_x','pos_y','pos_z'])
    return track_pos_world


if __name__ == "__main__":
    data_root = "F:/__Datasets__/OpenTraj/KITTI"
    kitti_root = f'{data_root}/data'

    dataset = load_kitti(kitti_root, title='kitti', use_kalman=True, sampling_rate=1)
    
    # postprocess apply `groupby` already ???
