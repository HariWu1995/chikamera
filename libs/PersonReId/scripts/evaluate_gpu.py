...
def compute_mAP(index, good_index, junk_index):
    ...
    for i in range(n_good):
        d_recall = 1. / n_good
        precision = (i+1) * 1. / (rows_good[i] + 1)
        if rows_good[i] != 0:
            old_precision = i * 1. / rows_good[i]
        else:
            old_precision = 1.0
...
if __name__ == "__main__":
    ...
    # Unit-test
    model_name = "ResNet50"
    data_dir = f"F:/__Datasets__/DukeMTMC/preprocessed/results_{model_name}"
    ...
    for i in tqdm(range(len(query_label))):
        ...
        mq_feature = torch.mean(mquery_feature[mquery_index, :], dim=0)
