...
from ..models import *
from ..losses import *
from ..dgfolder import DGFolder1
...
parser.add_argument('--DG', action='store_true', help='use extra DG-Market Dataset for training. '
...
dataloaders = {
        x: DataLoader(image_datasets[x], batch_size=opt.batch_size,
                        shuffle=True, sampler=img_sampler[x],
...
if opt.DG:
    ...
    image_datasets['DG'] = DGFolder(os.path.join('../DG-Market'), data_transforms['train'])
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for i, data in enumerate(dataloaders[phase]):
                ...
                ordered_dict["Acc"] = f"{(float(torch.sum(preds == labels.data)) / B):.4f}"
            ...
            # deep copy the model
            if opt.local_rank == 0 and phase == 'val' \
            and ((epoch+1) % 10 == 0 or epoch == num_epochs-1):
