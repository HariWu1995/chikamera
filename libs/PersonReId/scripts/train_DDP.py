...
def train_model(opt, dataloaders, dataset_sizes, 
    ...
    def save_model(model, suffix):
        if len(opt.gpu_ids) > 1:
            save_network(model.module, ckpt_dir, suffix, opt.local_rank)
        else:
            save_network(model, ckpt_dir, suffix, opt.local_rank)
    ...
    for epoch in range(opt.total_epoch):
        ...
        for phase in ['train', 'val']:
            ...
            for i, data in enumerate(dataloaders[phase]):
                ...
                return_feature = opt.arcface or opt.cosface or opt.circle or opt.triplet or \
...
def run(opt):
    ...
    image_samplers = {
            x: DistributedSampler(image_datasets[x]) 
        for x in ['train', 'val']
    }

    dataloaders = {
            x: DataLoader(image_datasets[x], batch_size=opt.batch_size,
                                shuffle=True, sampler=image_samplers[x],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--enhance_DG', default=True, type=bool, help='use synthetic DG-Market Dataset for training. '
                                                                    'Please download it from https://github.com/NVlabs/DG-Net#dg-market.')
    # optimizer & trainer
    ...
    parser.add_argument('--cosine', action='store_true', help='use cosine lr')
    ...
    parser.add_argument('--use_dense', action='store_true', help='use densenet-121')
    ...
    parser.add_argument('--aiter', default=10, type=float, help='use adv loss with iteration')
