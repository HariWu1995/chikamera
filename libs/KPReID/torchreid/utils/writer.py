...
from .avgmeter import TorchTimeMeter, SingleMeter, EpochMeter, EpochArrayMeter, LossEpochMetricsMeter
...
class Writer(EngineStateListener):
    ...
    def visualize_rank(self, test_loader, dataset_name, distmat, save_dir, visrank_topk, visrank_q_idx_list, visrank_count,
                        body_parts_distmat, qf_parts_visibility, gf_parts_visibility, q_parts_masks, g_parts_masks, q_pids, g_pids, q_camids, g_camids, q_anns, g_anns, eval_metrics):
        if self.cfg.test.visrank:
            ...
            visualize_ranking_grid(distmat, body_parts_distmat, test_loader, dataset_name,
                           qf_parts_visibility, gf_parts_visibility, q_parts_masks, g_parts_masks, q_pids, g_pids, q_camids, g_camids, q_anns, g_anns, eval_metrics, save_dir, visrank_topk, visrank_q_idx_list, visrank_count, config=self.cfg)
            ...
            if self.cfg.test.visrank_per_body_part:
                for bp in range(0, body_parts_distmat.shape[0]):
                    ...
                    visualize_ranking_grid(body_parts_distmat[bp], body_parts_distmat[bp:bp+1], test_loader, dataset_name,
                                   qf_part_visibility, gf_part_visibility, q_parts_masks, g_parts_masks, q_pids, g_pids, q_camids, g_camids, q_anns, g_anns, eval_metrics, save_dir, visrank_topk, visrank_q_idx_list, visrank_count, config=self.cfg, bp_idx=bp)
    ...
    def training_completed(self):
        print("Training completed in {}".format(self.epoch_timer.total_time()))
