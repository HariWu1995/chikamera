...
class Engine(object):
    ...
    def run(
        ...
        Args:
            ...
            save_feats_and_ids (bool, optional): save test query and test gallery extracted features and ids to disk
            load_feats_and_ids (bool, optional): load test query and test gallery extracted features and ids from disk
        """
        if test_only:
            self.test(
                0,
                ...
                ranks=ranks,
                rerank=rerank,
                save_feats_and_ids=save_feats_and_ids,
                load_feats_and_ids=load_feats_and_ids,
            )
            return
        ...
        for epoch in range(self.engine_state.start_epoch, self.engine_state.max_epoch):
            ...
            self.train(fixbase_epoch=fixbase_epoch, open_layers=open_layers)
    ...
    def test(
        ...
        self.writer.report_global_performance(cmc_per_dataset, mAP_per_dataset, 
                                            ssmd_per_dataset, pxl_acc_per_dataset)
    ...
    def _evaluate(
        ...
        different_ids_dist_mean, different_ids_dist_std, ssmd = \
            plot_pairs_distance_distribution(distmat, q_pids, g_pids, "Query-gallery")  # TODO separate ssmd from plot, put plot in writer
