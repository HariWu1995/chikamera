...
class Dataset(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.filter_out_no_skeletons:
            ...
            print('\nFiltered out {} / {} samples without keypoints'.format(or_size - len(self.train), or_size))
    ...
    def transforms(self, mode):
        ...
        if mode == 'train':
            ...
            raise ValueError(
                f"Invalid mode. Got {mode}, but expected to be 'train', 'query' or 'gallery'"
            )
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):  # kept for backward compatibility
        ...
        return self.getitem(sample, self.cfg, self.keypoints_to_prompt_masks, self.prompt_preprocess, 
                                              self.keypoints_to_target_masks, self.target_preprocess, 
                                            self.transforms(self.mode), self.load_masks, batch_pids, all_occluders)
    @staticmethod
    def getitem(sample, cfg, keypoints_to_prompt_masks, prompt_preprocess, keypoints_to_target_masks, target_preprocess, transforms, load_masks=False, batch_pids=None, all_occluders=None):
        ...
        This method will load the training / test samples, 
        i.e. the reid image, the keypoints, and the human parsing labels (often called 'masks').
        
        Image, keypoints and masks, then, undergo the configured transformations 
        (e.g. resize, crop, flip, BIPO to generate random person occlusion, ...).
        Albumentation is employed as a data augmentation library 
        to handle coherent transformation across the three loaded values.
        ...
        # needed for batch aware person copy paste to generate random occlusions
        if all_occluders:
        ...
        if kp_enabled:
            if 'keypoints_xyc' in sample or 'kp_path' in sample:
                assert not (keypoints_to_prompt_masks.mode == 'none' \
                        and keypoints_to_target_masks.mode == 'none'), "Error"

                if 'kp_path' in sample:  
                    # for standard reid datasets
                    assert 'keypoints_xyc' not in sample
                    kp_path = sample['kp_path']
                    bbox_ltwh = (0, 0, transf_args["image"].shape[1], 
                                       transf_args["image"].shape[0])
                    keypoints_xyc = read_keypoints(kp_path, bbox_ltwh)
                    keypoints_shape = keypoints_xyc.shape
                    keypoints_xyc = keypoints_xyc.reshape(keypoints_xyc.shape[0] * keypoints_xyc.shape[1], -1)
                else:  
                    # for Occ-PoseTrack21-ReID
                ...
                # if keypoints enabled but no keypoints provided as input, 
                # create empty keypoints as it is required by the transforms.
                n_kp = 17
        ...
        if kp_enabled:
            spatial_feature_size = (spatial_feature_shape[1], 
                                    spatial_feature_shape[0])  # = (W, H)
            ...
            if len(keypoints) > 0:
                keypoints_xyc[kp_indices] = np.concatenate((np.array(keypoints), 
                                            np.expand_dims(np.array(kp_vis_score), axis=1)), axis=1)
            ...
            if masks_prompting and keypoints_to_prompt_masks.mode != 'none':
                ...
                if use_negative_keypoints:
                    ...
                    if negative_keypoints_xyc.shape[0] > max_skeletons:
                        ...
                        sample["negative_keypoints_xyc"] = np.concatenate((negative_keypoints_xyc, 
        ...
        if masks_enabled and 'masks' in result:
            ...
            if not kp_enabled \
                or (kp_enabled and keypoints_to_target_masks.mode == 'none'):
                sample["target_masks"] = masks[0]
            if masks_prompting:
                if not kp_enabled \
                    or (kp_enabled and keypoints_to_prompt_masks.mode == 'none'):
