...
def eval_market1501(distmat, q_pids, g_pids, q_camids, g_camids, max_rank, q_anns, g_anns):
    ...
    if ordering_criteria not in q_anns:
        return results
    ...
    cmc_per_vis = []
    count_per_vis = []
    sample_vis_scores = q_anns[ordering_criteria]

    if len(queries) >= 10:

        if ordering_criteria == "visibility":
            ranges = [(i * 0.1, (i+1) * 0.1) for i in range(0, 10)]

        elif ordering_criteria == "occ_level":
        ...
        for (low_bd, up_bd) in ranges:
            samples_in_range = np.logical_and(sample_vis_scores > low_bd, 
                                              sample_vis_scores <= up_bd)
            if np.any(samples_in_range):
                vis_mAP = np.mean(all_AP[samples_in_range])
                vis_cmc = all_cmc[samples_in_range].sum(0) / samples_in_range.sum()
            else:
                vis_mAP = None
                vis_cmc = None
...
def eval_motchallenge(distmat, q_pids, g_pids, q_camids, g_camids, max_rank, q_anns, g_anns):
    ...
    if q_anns is not None \
    and g_anns is not None and 'visibility' in q_anns:
...
def eval_generic(distmat, q_pids, g_pids, q_camids, g_camids, max_rank, q_anns, g_anns, gallery_filter=None):
    """
    Generic evaluation with market1501 metric given a function 
        to filter gallery samples based on given query sample
    """
    num_q, num_g = distmat.shape
    ...
    ordering_criteria = "occ_level"
    if q_anns is not None \
    and g_anns is not None and ordering_criteria in q_anns:
        queries = []
        for i, (ap, r1) in enumerate(zip(all_AP, all_cmc)):
            query_sample = {
                'index': i,
