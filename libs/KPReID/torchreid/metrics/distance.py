...
def compute_distance_matrix_using_bp_features(qf, gf, qf_parts_visibility=None, gf_parts_visibility=None, dist_combine_strat='mean', batch_size_pairwise_dist_matrix=5000, use_gpu=False, metric='euclidean', use_logger=True):
    """
    Computes distance matrix between each pair of samples using their part-based features. 
    3 implementations here: 
        - without visibility scores, 
        - with boolean / binary visibility scores
        - with continuous [0, 1] visibility scores.
    """
    arguments = [
        qf, gf, qf_parts_visibility, gf_parts_visibility, 
        dist_combine_strat, batch_size_pairwise_dist_matrix, 
        use_gpu, metric, use_logger
    ]

    # TODO: keep only one generic implementation
    if qf_parts_visibility is not None \
    and gf_parts_visibility is not None:
        if qf_parts_visibility.dtype is torch.bool \
        and gf_parts_visibility.dtype is torch.bool:
            # boolean visibility scores
            return _compute_distance_matrix_using_bp_features_and_masks(*arguments)
        else:
            # continuous visibility scores
            return _compute_distance_matrix_using_bp_features_and_visibility_scores(*arguments)
    else:
        # no visibility scores
        arguments = arguments[:2] + arguments[4:]
        return _compute_distance_matrix_using_bp_features(*arguments)
