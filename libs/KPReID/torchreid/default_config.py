...
def get_default_config():
    ...
    cfg.model.pretrained = True     # load pretrained model weights if available (For example HRNet pretrained weights on ImageNet)
    ...
    cfg.model.load_config = True    # load config saved with model weights and overwrite current config
    ...
    cfg.model.kpr.normalization = "identity"  # ['identity', 'batch_norm_2d'] - obsolete, always use identity
    ...
    cfg.model.kpr.last_stride = 1  # last stride of the resnet backbone - 1 for better performance
    ...
    cfg.model.kpr.dim_reduce_output = 512  # reduce feature dimension to this value when above config is not 'none'
    ...
    cfg.model.kpr.masks.background_computation_strategy = "threshold"  # threshold, diff_from_max
    ...
    cfg.data.workers = 0  # set to 0 to enable breakpoint debugging in dataloader code
    ...
    cfg.data.bipo = CN()# parameters for random occlusion data augmentation with Pascal VOC, to be improved, not maintained
    cfg.data.bipo.path = ""
    cfg.data.bipo.p = 0.2
    cfg.data.bipo.n = 1
    ...
    cfg.data.mot = CN() # Config for building a ReID dataset from a MOT dataset, not maintained
    ...
    # loss
    cfg.loss = CN()
    cfg.loss.name = "part_based"  # use part based engine to train kpr with GiLt loss
    ...
    # To avoid out of memory issue, we don't compute it for all gallery samples at the same time,
    # but we compute it in batches of 'batch_size_pairwise_dist_matrix' gallery samples.
    ...
    cfg.test.save_feats_and_ids = True  # save test set extracted data to disk
    cfg.test.load_feats_and_ids = False  # load test set extracted data from disk. NOTE: you HAVE TO run without this flag first
    cfg.test.detailed_ranking = False  # display ranking performance for each part individually
...
def engine_run_kwargs(cfg):
    return {
        ...
        "save_feats_and_ids": cfg.test.save_feats_and_ids,
        "load_feats_and_ids": cfg.test.load_feats_and_ids,
    }
