...
def centroid_distance(tracks, detections, metric='euclidean'):  # 추가
    ...
    if cost_matrix.size == 0:
        return cost_matrix
    det_centroids = np.asarray([track.centroid for track in detections], dtype=np.float64)
    track_centroids = np.asarray([track.centroid for track in tracks], dtype=np.float64)
