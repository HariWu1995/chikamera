...
class SpeedAndDistanceCalculator():
    ...
    def __init__(self, width_in_pixels, height_in_pixels,
                       width_in_meters, height_in_meters, **kwargs):
    ...
    def calculate_distance(self, tactical_player_positions):
        ...
        for frame_number, tactical_player_position_frame in enumerate(tactical_player_positions):
            ...
            for player_id, current_player_position in tactical_player_position_frame.items():
                # Calculate distance
                if player_id in prev_players_position:
                    prev_position = prev_players_position[player_id]
                    meter_distance = self.calculate_meter_distance(prev_position, current_player_position)
                    output_distances[frame_number][player_id] = meter_distance

                prev_players_position[player_id]=current_player_position
        
        return output_distances

    def calculate_meter_distance(self, prev_pixel_position, curr_pixel_position):
        # Calculate the meter distance betweent current position and prev position
        prev_pixel_x, prev_pixel_y = prev_pixel_position
        curr_pixel_x, curr_pixel_y = curr_pixel_position
        ...
        curr_meter_x = curr_pixel_x * self.width_in_meters  / self.width_in_pixels
        curr_meter_y = curr_pixel_y * self.height_in_meters / self.height_in_pixels

        meter_distance = measure_distance((curr_meter_x, curr_meter_y),
                                          (prev_meter_x, prev_meter_y))
