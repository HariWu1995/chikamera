...
class TeamBallControlDrawer:
    ...
    def draw_frame(self, frame, frame_num, team_ball_control):
        ...
        draw_params = (cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0,0,0), font_thickness)

        cv2.putText(frame, f"Team 1 Ball Control: {team_1*100:.2f}%", (text_x, text_y1), *draw_params)
        cv2.putText(frame, f"Team 2 Ball Control: {team_2*100:.2f}%", (text_x, text_y2), *draw_params)
