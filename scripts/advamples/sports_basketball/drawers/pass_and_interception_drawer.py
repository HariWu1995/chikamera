...
class PassInterceptionDrawer:
    ...
    def get_stats(self, passes, interceptions):
        ...
        Args:
            passes (list): A list of integers representing pass events at each frame.
                           (1 represents a pass by Team 1, 
                            2 represents a pass by Team 2, 
                            0 represents no pass.)
            interceptions (list): A list of integers representing interception events at each frame.
                           (1 represents an interception by Team 1, 
                            2 represents an interception by Team 2, 
                            0 represents no interception.)

        Returns:
            tuple: A tuple of four integers (team1_pass_total, team2_pass_total,
                    team1_interception_total, team2_interception_total) indicating 
                    the total number of passes and interceptions for both teams.
        """
        team1_passes = []
        team2_passes = []
        team1_interceptions = []
    ...
    def draw_frame(self, frame, frame_num, passes, interceptions):
        ...
        team2_passes, \
        team1_interceptions, \
        team2_interceptions = self.get_stats(passes_till_frame, 
                                      interceptions_till_frame)

        annot_params = (cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0,0,0), font_thickness)

        msg_1 = f"Team 1 - Passes: {team1_passes} Interceptions: {team1_interceptions}"
        msg_2 = f"Team 2 - Passes: {team2_passes} Interceptions: {team2_interceptions}"

        cv2.putText(frame, msg_1, (text_x, text_y1), *annot_params)        
        cv2.putText(frame, msg_2, (text_x, text_y2), *annot_params)
