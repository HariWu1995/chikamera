...
def main(
    ...
    def inference_callback(frame: VideoFrame) -> sv.Detections:
        results = model(frame.image, verbose=False, conf=confidence_threshold, device=device)[0]
        return resultssv.Detections.from_ultralytics(results).with_nms(threshold=iou_threshold)
